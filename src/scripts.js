/*  Create a class called ToDoList
    ...
    -   Add the generateTaskHtml method (copy from below)
        ...
        -   Rps phrases with tempeplace the other 2 ALL calate strings
...
class ToDoList{
          ...
          <li class="list-group-item checkbox">
            <div class="row">
              <div class="col-md-1 col-xs-1 col-lg-1 col-sm-1 checkbox">
                ...
                value="" class="" ${(task.isComplete)?'checked':''}></label>
              </div>
              <div class="col-md-10 col-xs-10 col-lg-10 col-sm-10 task-text ${(task.isComplete)?'complete':''}">
                ${task.task}
              </div>
              <div class="col-md-1 col-xs-1 col-lg-1 col-sm-1 delete-icon-area">
                <a class="" href="/" ><i id="deleteTask" class="delete-icon glyphicon glyphicon-trash"
                onClick = "toDo.deleteTask(event, ${index})"></i></a>
              </div>
            </div>
          </li>
        `;
    ...
    addEventListeners(){
        document.getElementById('addTask').addEventListener('keypress', 
        event => {
            if(event.keyCode === 13) {
